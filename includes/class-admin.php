<?php
/**
 * File containing the class \Sensei_LMS_Beta_Tester\Admin.
 *
 ...
 *
 * @class \Sensei_LMS_Beta_Tester\Admin
 */
final class Admin {
		...
		if ( false === $current_version ) {
			add_action(
				...
				function( $plugin_meta, $plugin_file ) {
					...
					$message      .= esc_html__( 'Requires Sensei LMS to be installed and activated.', 'sensei-lms-beta-tester' );
					$message      .= '</span>';
					$plugin_meta[] = $message;
	...
	public function handle_version_switch() {
		...
		// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Keep the nonce.
		if ( ! wp_verify_nonce( wp_unslash( $_POST['_wpnonce'] ), 'switch-sensei-lms-version' ) ) {
			wp_die( esc_html__( 'Action failed. Please go back and retry.', 'sensei-lms-beta-tester' ) );
			return;
		}

		$new_version = isset( $_POST['sensei_lms_beta_version_select'] ) ? sanitize_text_field( wp_unslash( $_POST['sensei_lms_beta_version_select'] ) ) : false;
		...
		if ( empty( $error_message ) && false === $result ) {
			...
			$result = sprintf( esc_html__( 'An error occurred while attempting to switch the version to %s', 'sensei-lms-beta-tester' ), $new_version );
		}
		...
		wp_safe_redirect( admin_url( 'plugins.php?page=sensei-lms-beta-tester-tester' ) );
		exit;
	}
	...
	public function init_settings() {
		register_setting( 'sensei-lms-beta-tester', 'sensei_lms_beta_options' );

		add_settings_section(
			'sensei-lms-beta-tester-update',
			__( 'Settings', 'sensei-lms-beta-tester' ),
			[ $this, 'update_section_html' ],
			'sensei-lms-beta-tester'
		);

		add_settings_field(
			'sensei-lms-beta-tester-channel',
			__( 'Release Channel', 'sensei-lms-beta-tester' ),
	...
	public function update_section_html( $args ) {
		?>
		<p id="<?php echo esc_attr( $args['id'] ); ?>"><?php esc_html_e( 'The following settings allow you to choose which Sensei updates to receive on this site, including beta and RC versions not quite ready for production deployment.', 'sensei-lms-beta-tester' ); ?></p>
		<?php
	}
	...
	public function version_select_html( $args ) {
		...
		$channels = [
			Updater::CHANNEL_BETA   => [
				'name'        => __( 'Beta Releases', 'sensei-lms-beta-tester' ),
				'description' => __( 'Beta releases contain experimental functionality for testing purposes only. This channel will also include RC and stable releases if more current.', 'sensei-lms-beta-tester' ),
				'latest'      => Updater::instance()->get_latest_channel_release( Updater::CHANNEL_BETA ),
			],
			Updater::CHANNEL_RC     => [
				'name'        => __( 'Release Candidates', 'sensei-lms-beta-tester' ),
				'description' => __( 'Release candidates are released to ensure any critical problems have not gone undetected. This channel will also include stable releases if more current.', 'sensei-lms-beta-tester' ),
				'latest'      => Updater::instance()->get_latest_channel_release( Updater::CHANNEL_RC ),
			],
			Updater::CHANNEL_STABLE => [
				'name'        => __( 'Stable Releases', 'sensei-lms-beta-tester' ),
				'description' => __( 'This is the default behavior in WordPress.', 'sensei-lms-beta-tester' ),
				'latest'      => Updater::instance()->get_latest_channel_release( Updater::CHANNEL_STABLE ),
			],
		];
		echo '<fieldset><legend class="screen-reader-text"><span>' . esc_html__( 'Update Channel', 'sensei-lms-beta-tester' ) . '</span></legend>';
		foreach ( $channels as $channel_id => $channel ) {
			...
			<label>
				...
				$update_time = ( $channel['latest'] && $channel['latest']->get_release_date() )
					...
					? sprintf( __( 'Last updated %s ago', 'sensei-lms-beta-tester' ), human_time_diff( strtotime( $channel['latest']->get_release_date() ) ) )
					: false;
				?>
	...
	public function add_admin_menu() {
		add_plugins_page( esc_html__( 'Sensei Beta Tester', 'sensei-lms-beta-tester' ), esc_html__( 'Sensei Beta Tester', 'sensei-lms-beta-tester' ), 'install_plugins', 'sensei-lms-beta-tester-tester', [ $this, 'output_settings_page' ] );
	}
	...
	public function add_settings_link( $actions, $plugin_file ) {
		...
		$new_actions['settings'] = sprintf(
			'<a href="%s">%s</a>',
			esc_url( admin_url( 'plugins.php?page=sensei-lms-beta-tester-tester' ) ),
			esc_html__( 'Settings', 'sensei-lms-beta-tester' )
		);
